...
A feature map rots the moment the app changes. This skill is the upkeep loop for a skill generated by `$forge-create-verification-skill` (or any project-local verification skill with a feature map). The unit of rigor is the feature, not every sentence: cover every feature file from source and exercise every feature live, without terminalising every bullet.

Automatic selection follows the shared checkpoint contract in the installed `engineering-playbooks/references/agent-friendly-workflow.md`.
A targeted recipe repair is not a completed full maintenance pass. Full coverage
accounts for every mapped entry point and sub-feature on its declared platform;
merge overlapping actions only when their observable claims are actually covered.
Project launch contracts override the generic long-lived-instance suggestion below:
use separate governed launches whenever target or source identity changes.
...
- **changed** — proven local corrections and completed mapped coverage; publication only if authorized.
- **blocked** — coverage could not finish or a proven fix could not ship safely. Retain proven partial corrections, list their evidence separately, and say exactly what remains blocked.
...
6. **Ship or stop.** For changed: inspect and validate every local correction; prepare one PR only with publication authority. For clean or blocked: no PR, report the outcome and the coverage honestly.
...
Group source inspections into the smallest useful assignments through the orchestrator and its concurrency cap. A worker does not spawn another worker. Runtime verification needs the matching surface; a verified prerequisite is not a verified feature. Apply local authorized corrections; a PR is only created with publication authority.
