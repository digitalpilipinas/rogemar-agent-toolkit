...
For broad security reviews, the orchestrator may assign one bounded, read-only
worker to inspect a defined language/framework surface or invariant. Require
file-and-line evidence, exploitability or impact reasoning, and clear
remediation boundaries. Pass the parent model and reasoning ceilings; the
security worker may use the strongest permitted profile but never a stronger
model or higher effort than the main agent. The main agent reconciles findings
and decides whether fixes are authorized.
