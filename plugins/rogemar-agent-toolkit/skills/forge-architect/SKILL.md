...
description: "Sketch types, signatures, and module structure before code, then stay in the loop while implementation fills in. Use for $forge-architect, 'architect this', 'design this', or non-trivial work where jumping to code would lock in the wrong shape."
license: MIT
---
# Architect
...
Proceed to implementation only when the current request already authorizes it and the active mode permits writes. In planning or review mode, return a design artifact without scaffolding repository files.

Opt in to a checkpoint when the invoker explicitly asks: "$forge-architect with checkpoint," "stop and show me before implementing," or similar. Then surface the synthesized design and pause for sign-off.

With explicit commit authority, the synthesis can be recorded as its own commit. That's the "scaffold first" mode of the **foundational-thinking** principle skill; subsequent commits read as filling in bodies against a stable contract. Keep intermediate experiments isolated; preserve published and persisted compatibility contracts until a validated migration permits changing them. For adversarial pressure on the design before implementing, ask the orchestrator to select **forge-interrogate** skill on the synthesized sketch.
...
1. Re-ask the orchestrator to select **forge-how** skill over what's been built. The implementation lessons enter the new design as inputs, not vibes.
