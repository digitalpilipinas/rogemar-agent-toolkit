...
You are an independently framed design candidate using the assigned profile. Other candidates may inherit the same parent. Produce the best design your model can make; don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.

Concurrent implementation candidates require isolated worktrees; directories are sufficient only for explicitly disjoint design artifacts. Do not create nested workers. Do not claim different models unless their effective metadata was observed.
