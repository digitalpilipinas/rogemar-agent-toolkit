...
Use the Figma MCP only when it is surfaced and authenticated for the current task. If it is unavailable, work from user-provided screenshots, exports, node context, or repository artifacts where sufficient; otherwise state the limitation and request the smallest needed artifact or authorization. Do not install, authenticate, or claim design parity automatically.
