...
- Principle: a sentence on what the agent should have looked up only within explicit authorization.
...
- `Read` tool calls against any `SKILL.md` file (workspace `.agents/skills/`, user-level `~/.codex/skills/`, or plugin-installed paths under `the verified installed skill paths/`)
- `workflow-orchestrator` delegation prompts that name a skill path
