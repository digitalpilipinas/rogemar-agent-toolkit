...
2. `forge-how` to ground hypotheses; don't claim a perf ceiling without running it first.
...
3. Plan the fix from the trace. Use `forge-architect` for a consequential design choice. Implement directly or request a bounded `perf-issue` assignment through the orchestrator; review the actual diff. Capture a post-fix trace.
