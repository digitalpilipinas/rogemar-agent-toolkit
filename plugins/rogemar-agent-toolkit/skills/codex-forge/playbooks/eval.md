...
- No `eval`, `test`, `judge`, `experiment`, `rubric`, `score`, `compare`, `benchmark`, `candidate`, or `forge-arena` in any directory, file, or prompt the candidate sees.
...
4. **Request bounded independent candidates through the orchestrator** using available router-qualified profiles under the active mode (parent ceilings without a mode), preserving explicit user and host limits, per the **forge-arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Request one read-only blinded judge through the orchestrator** with an independent lens per the **forge-arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify behavior from artifacts and available tool evidence.** Use only exact task-specific traces supplied by the runtime or verified through task metadata, scoped to the authorized experiment. Never scan unrelated history. If tool-read evidence is unavailable, report chain-following as unverified; citing a skill does not prove reading or applying it. Do not invent model identity from requested parameters.
