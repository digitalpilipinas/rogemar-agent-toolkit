...
3. Model the domain before branching logic. Use a state machine, typed model or registry when it removes invalid states or repeated assumptions. Preserve existing contracts and nearby conventions. Use arena for materially competing designs, not for every edit.
...
5. Reconcile all consumers of changed shared primitives. Revisit design if repeated workarounds show the model is wrong. Use `forge-interrogate` where a consequential unresolved assumption needs critique.
