...
describe("checks fallback chain", () => {
  it("uses the complete rollup even when the fast path reports success", async () => {
    ...
    expect(reader.calls).toEqual(["checkRollupPage:null"]);
  });
...
describe("rollup node mapping", () => {
  ...
  it("maps StatusContext states and rejects unknown typenames", () => {
    expect(
      mapRollupNode({
        __typename: "StatusContext",
        context: "ci",
...
it("annotates Bugbot threads with distinct review-pass counts", () => {
  ...
  expect(threads.map((thread) => thread.bugbotReviewPasses)).toEqual([3, 3, 3]);
});
