...
export function fakeReader(
  ...
  return {
    calls,
    async reviewEvidence(_context, head) {
      calls.push("reviewEvidence");
      return {head, capturedAt: "2026-01-01", complete: true, comments: [], reviews: [], requests: [], annotations: []};
    },
    async originRepo() {
    ...
    async checkRollupPage(_requested, after) {
      ...
      if (options.rollupPages) {
        if (after === null) page = 0;
        const result = options.rollupPages[page++];
        if (!result) throw new Error("missing fixture page");
        return result;
      }
      return {checks: options.fastPath?.kind === "checks" ? options.fastPath.checks : options.fastPath ? [] : [passingCheck()], endCursor: null};
    },
    async reviewThreads() {
