import {allThreads, connection, COMMITS, collectReviewEvidence, restPages, type JsonQuery} from "./evidence.ts";
...
async function runJson(argv: readonly [string, ...string[]]): Promise<unknown> {
  ...
  const value = parseJson(result.stdout, argv.join(" "));
  if (isRecord(value) && value.errors && (!Array.isArray(value.errors) || value.errors.length)) missing("GraphQL errors", value.errors);
  return value;
}
...
function pendingOrGate(details: {readonly name: string; readonly description: string; readonly link: string; readonly workflow: string}, reportedState: string): T.Check {
  return {...details, kind: "pending", reportedState};
}
export function mapRollupNode(value: unknown): T.Check | null {
  const object = record(value, "rollup node");
  const typename = object.__typename;
  if (typename !== "CheckRun" && typename !== "StatusContext") return missing("unknown check node", value);
...
export function parseReviewThreads(value: unknown): readonly T.ReviewThread[] {
  ...
  const threads: {
    ...
    readonly comments: readonly T.ReviewComment[];
  ...
  return threads
    .map(({ id, firstComment, comments, resolved }) => ({
      comments, resolved,
      id,
      firstComment,
...
export class GhGitHubReader implements T.GitHubReader {
  constructor(private readonly query: JsonQuery = runJson) {}
  async reviewEvidence(c: T.PrContext, head: string) {
    try { return await collectReviewEvidence(this.query, c, head); }
    catch (error) { throw new ChecksUnavailable(String(error)); }
  }
  ...
  async pullRequest(context: T.PrContext): Promise<T.PullRequestFacts> {
    return parsePullRequest(
      await this.query([
        ...
        "--json",
        "mergeable,mergeStateStatus,reviewDecision,headRefOid,baseRefOid,headRefName,baseRefName,state,mergedAt,isDraft",
      ]),
      context
    );
  }
  async openPullRequests(repository: T.Repository): Promise<readonly T.OpenPullRequest[]> {
    try {
      const rows = await restPages(this.query, `repos/${repository.owner}/${repository.repo}/pulls?state=open&per_page=100`);
      return rows.filter(r => r.head?.repo?.full_name === `${repository.owner}/${repository.repo}`).map(r => ({
        number: parsePrNumber(r.number), headRefName: string(r.head?.ref, "head.ref"), baseRefName: string(r.base?.ref, "base.ref")
      }));
    } catch (error) { throw new ChecksUnavailable(String(error)); }
  }
  ...
  async checkRollupPage(
    ...
    const value = await this.query(argv);
    const commits = list(
      at(value, ["data", "repository", "pullRequest", "commits", "nodes"]),
      "commits.nodes"
    );
    if (commits.length !== 1) throw new ChecksUnavailable("missing expected-head commit during pagination");
    const commit = record(
      at(commits[commits.length - 1], ["commit"]),
      "commit"
    );
    if (expectedHead !== undefined && commit.oid !== expectedHead) throw new ChecksUnavailable("head changed during check collection");
    if (commit.statusCheckRollup === null)
      throw new ChecksUnavailable("missing current-head check rollup during pagination");
    ...
    if (page.hasNextPage && !cursor) throw new ChecksUnavailable("missing check pagination cursor");
  ...
  async reviewThreads(context: T.PrContext): Promise<readonly T.ReviewThread[]> {
    try {
      const nodes = await allThreads(this.query, context);
  ...
  async commitRollups(context: T.PrContext): Promise<readonly T.CommitRollup[]> {
    try {
      const rows = await connection(this.query, COMMITS, context, d => d?.repository?.pullRequest?.commits);
      return rows.map(r => ({oid: string(r.commit?.oid, "commit.oid"), state: r.commit.statusCheckRollup === null ? null : nullableEnum(r.commit.statusCheckRollup?.state, ROLLUP_STATES, "rollup.state")}));
    } catch (error) { throw new ChecksUnavailable(String(error)); }
  }

}

export async function resolveChecks(reader: T.GitHubReader, context: T.PrContext, expectedHead?: string): Promise<T.CheckRead> {
  const checks: T.Check[] = [], seen = new Set<string>();
  let after: string | null = null;
  do {
    const page = await reader.checkRollupPage(context, after, expectedHead);
    checks.push(...page.checks);
    after = page.endCursor;
    if (after !== null) {
      if (!after || seen.has(after)) throw new ChecksUnavailable("repeated check pagination cursor");
      seen.add(after);
    }
  } while (after !== null);
  const complete = nonEmpty(checks);
  if (complete === null) throw new ChecksUnavailable("current-head check rollup empty; readiness unverified");
  return {source: "graphql-rollup", checks: complete};
}
...
export function orderStack(
  ...
  if (byHead.size !== open.length) throw new ChecksUnavailable("ambiguous stack head branches");
  ...
  let current = start;
  const ancestors = new Set([current.headRefName]);
  while (byHead.has(current.baseRefName)) {
    if (ancestors.has(current.baseRefName)) throw new ChecksUnavailable("cycle in stack bases");
    ancestors.add(current.baseRefName);
