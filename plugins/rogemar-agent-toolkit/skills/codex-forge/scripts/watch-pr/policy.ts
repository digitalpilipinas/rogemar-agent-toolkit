...
export function assessGitHubMerge(args: {
  ...
  if ((args.mergeStateStatus === "CLEAN" || args.mergeStateStatus === "HAS_HOOKS") && args.headRollupState === "SUCCESS")
    return {kind: "allowed", basis: "merge-state", mergeStateStatus: args.mergeStateStatus, headRollupState: args.headRollupState};
  return {kind: "refused", mergeStateStatus: args.mergeStateStatus, headRollupState: args.headRollupState};
}
...
export function sameReadinessFacts(a: T.PullRequestFacts, b: T.PullRequestFacts): boolean {
  return a.headRefOid === b.headRefOid && a.baseRefOid === b.baseRefOid &&
    a.headRefName === b.headRefName && a.baseRefName === b.baseRefName && a.state === b.state &&
    a.mergedAt === b.mergedAt && a.mergeable === b.mergeable && a.mergeStateStatus === b.mergeStateStatus &&
    a.reviewDecision === b.reviewDecision && a.isDraft === b.isDraft;
}
export async function readSnapshot(args: {
  ...
  if (!facts.headRefOid || !facts.baseRefOid) throw new WatcherQueryError({kind: "checks-unavailable", retryable: true, detail: "missing current head SHA"});
  const evidence = await args.reader.reviewEvidence(args.context, facts.headRefOid);
  const threads = await args.reader.reviewThreads(args.context);
  const checks = await resolveChecks(args.reader, args.context, facts.headRefOid);
  ...
  const finalFacts = await args.reader.pullRequest(args.context);
  if (!sameReadinessFacts(finalFacts, facts))
    throw new WatcherQueryError({kind:"checks-unavailable", retryable:true, detail:"PR changed during evidence collection"});
  return {
    kind: "open",
    evidence,
...
function threadBlocker(row: T.PrSnapshot): T.MergeBlocker | null {
  ...
  const threads = nonEmpty(row.threads.filter(t => !t.resolved));
  return threads === null
    ? null
...
function gateReason(
  ...
  if (row.facts.mergeable !== "MERGEABLE") return "mergeability-unknown";
...
function readyContribution(
  ...
  return {
    ...
    proof: {
      ...
      base: {name: row.facts.baseRefName, sha: row.facts.baseRefOid},
      reviewThreads: row.threads,
      threads: [],
      ci: row.ci,
      gate: {
        state: "OPEN",
...
export async function runSimple(args: {
  ...
  const step = async (): Promise<StepResult<T.TerminalVerdict>> => {
    ...
    for (const row of complete) {
      const current = await args.dependencies.reader.pullRequest(row.context);
      if (!sameReadinessFacts(current, row.facts))
        throw new WatcherQueryError({kind:"checks-unavailable", retryable:true, detail:"stack changed during collection"});
    }
    const decision =
      args.mode === "single"
