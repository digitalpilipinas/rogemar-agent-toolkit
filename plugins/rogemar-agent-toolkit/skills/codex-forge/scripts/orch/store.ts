...
function resolveFrontier(repo: string, pin?: readonly number[]): readonly FrontierPr[] {
  const command = (exe: string, args: string[]) => {
    try { return execFileSync(exe, args, {cwd: repo, env: process.env, encoding: "utf8", maxBuffer: 32 * 1024 * 1024, stdio: ["ignore", "pipe", "pipe"]}).trim(); }
    catch (error) { throw new UserError(`frontier query failed: ${errorMessage(error)}`); }
  };
  let rows: any, meta: any;
  try {
    meta = JSON.parse(command("gh", ["repo", "view", "--json", "nameWithOwner,defaultBranchRef"]));
    if (typeof meta.nameWithOwner !== "string" || !/^[^/]+\/[^/]+$/.test(meta.nameWithOwner) || typeof meta.defaultBranchRef?.name !== "string") throw new Error("invalid repository identity");
    const records = command("gh", ["api", "--method", "GET", "--paginate", "--jq", ".[] | {number,state,merged_at,head:{ref:.head.ref,sha:.head.sha,repo:{full_name:.head.repo.full_name}},base:{ref:.base.ref}} | @json", `repos/${meta.nameWithOwner}/pulls?state=all&per_page=100`]);
    const pages = records.split(/\r?\n/).filter(line => line.trim()).map(line => JSON.parse(line));
    if (pages.some(row => !row || typeof row !== "object" || Array.isArray(row))) throw new Error("expected complete PR records");
    rows = pages.filter((r: any) => r?.head?.repo?.full_name === meta.nameWithOwner).map((r: any) => {
      if (!Number.isSafeInteger(r.number) || r.number <= 0 || !["open", "closed"].includes(r.state) ||
          typeof r.head?.ref !== "string" || typeof r.base?.ref !== "string" ||
          !/^[a-f0-9]{40,64}$/i.test(r.head?.sha ?? "") || typeof r.head?.repo?.full_name !== "string") throw new Error("invalid PR record");
      return {pr: r.number, branches: r.head.ref, base: r.base.ref, sha: r.head.sha,
        headRepo: r.head.repo.full_name,
        state: r.merged_at ? "MERGED" : r.state === "open" ? "OPEN" : "CLOSED"};
    });
  } catch (error) { throw new UserError(`invalid GitHub frontier evidence: ${errorMessage(error)}`); }
  const current = command("git", ["symbolic-ref", "--quiet", "--short", "HEAD"]);
  const chain: FrontierPr[] = [], seen = new Set<string>();
  let branch = current;
  while (branch !== meta.defaultBranchRef.name) {
    if (seen.has(branch)) throw new UserError("cycle in GitHub frontier");
    seen.add(branch);
    let matches = rows.filter((r: any) => r.branches === branch && r.headRepo === meta.nameWithOwner);
    if (matches.length > 1 && pin) matches = matches.filter((r: any) => pin.includes(r.pr));
    if (matches.length !== 1) throw new UserError(`ambiguous or missing PR for branch ${branch}`);
    const row = matches[0];
    const local = command("git", ["rev-parse", "--verify", "--end-of-options", `refs/heads/${branch}^{commit}`]);
    if (local !== row.sha) throw new UserError(`local/remote head mismatch for ${branch}`);
    chain.push({pr: row.pr, branches: row.branches, sha: row.sha, state: row.state});
    branch = row.base;
  }
  if (!chain.length) throw new UserError("current branch has no PR frontier");
...
function validateFrontierPin({
  ...
  if (extra.length > 0) {
    drift.push(`extra in GitHub: ${extra.join(",")}`);
  }
  ...
  return {
    ...
    frontier: {
      set: async (params) => {
        ...
        const prs = resolveFrontier(repo, pin);
        if (pin !== undefined) {
          validateFrontierPin({
        ...
        const value: Frontier = {
          ...
          prs,
          lowestUnmerged: prs.find((row) => row.state !== "MERGED")?.pr ?? null,
        };
        await atomicWrite(
