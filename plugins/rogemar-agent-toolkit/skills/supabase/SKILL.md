---
name: supabase
description: "Connector-independent Supabase skill and CLI/documentation fallback. Use when the namespaced Supabase plugin or its live tools are unavailable or unauthenticated, when the user explicitly requests the local skill/CLI route, or when declarative-schema guidance is required. Prefer the installed Supabase plugin when it is healthy."
metadata:
  author: supabase
  version: "0.1.2"
---
...
This global skill is the local fallback for the namespaced Supabase plugin. Do
not run both routes by default. Prefer the plugin when its skill and required
live tools are available and authenticated; use this skill for repository,
CLI, documentation, or declarative-schema work when that primary route is
unavailable or explicitly bypassed. Loading this skill does not itself grant
MCP access, authentication, migration authority, or production permission.
