...
Read [the Forge runtime contract](../codex-forge/references/codex-runtime.md). The orchestrator owns all reviewer requests; the router applies both manually selected parent ceilings. Another Codex model is optional and must be available. Independent agreement is supporting evidence, never a substitute for tracing the actual issue.
