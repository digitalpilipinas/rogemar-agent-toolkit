...
2. Review the actual diff and run relevant checks. Apply `comment review` only where comment cleanup helps and preserve necessary comments and suppressions. Use `technical writing` and `plain-language editing` for the description without imposing prose rules over the repository template.
