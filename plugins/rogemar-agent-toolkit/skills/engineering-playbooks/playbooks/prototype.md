...
The one playbook where the Laziness Protocol's "smallest change" and the verification bar invert. Optimize for the decision with a small isolated experiment; preserve security and privacy boundaries. In planning mode, describe the experiment and return a sketch without repository writes. The rigor is in picking the right design cheaply. Be bold: propose variations the user didn't ask for, throw an approach away and try another.
...
6. Present alternatives, tradeoffs, and a recommendation. The output is the decision plus the throwaway artifact, not shippable code. Hand the chosen direction to **Feature** (or `design exploration` for the shape) for the real build.
