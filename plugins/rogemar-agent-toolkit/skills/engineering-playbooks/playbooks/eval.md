...
- No `eval`, `test`, `judge`, `experiment`, `rubric`, `score`, `compare`, `benchmark`, `candidate`, or `blinded alternatives` in any directory, file, or prompt the candidate sees.
...
4. **Request bounded independent candidates through the orchestrator** using available profiles below both parent ceilings using the blinding rules in this playbook. Each works in its own sanitized dir; same prompt to each.
5. **Request one read-only blinded judge through the orchestrator** with an independent lens using the same blinded comparison. Judge sees outputs by sanitized label and the rubric, never a model name.
