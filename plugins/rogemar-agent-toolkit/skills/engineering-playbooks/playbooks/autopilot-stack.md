...
For each material verification round, record the candidate head, base, working-tree identity, affected checks and resulting evidence in the existing acceptance record. Re-run affected behavior after fixes; a successful earlier round does not prove the changed candidate. When a failure may be pre-existing, compare the same scenario on the declared baseline or current trunk with matched configuration, fixtures and measurement method. Record an absent feature as absent, not as a passing baseline. Carry a demonstrated shared defect into the relevant regression checks. Keep checks proportional to risk and preserve existing provider review allowances; rounds do not create new review budgets, fixed panels, publication authority or recurring wake loops.
