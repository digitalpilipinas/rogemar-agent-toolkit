...
## Prerequisites
- Use the Linear MCP only when its tools are surfaced and authenticated in the current environment.
- If it is unavailable, do not install or authenticate it automatically. Use local, user-provided, or repository evidence when sufficient; otherwise state the limitation and ask for authorization or an export.
