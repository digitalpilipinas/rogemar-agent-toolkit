---
name: forge-show-me-your-work
description: "Keep a local evidence-backed decision trail for long or multi-phase work. Audit actual results and apply publication authority separately."
license: MIT
---
# Show me your work
...
With explicit commit authority, include it when the work is ambitious enough that a reviewer needs the trail to trust the result: a large cross-language port, a multi-week migration, anything where confidence has to be shown rather than assumed. A committed log renders as a table in the PR.

## Rules

- One row is one decision or checkpoint. If it doesn't fit on one line, the decision isn't crisp yet.
- Append-only. A wrong call gets a new row that supersedes it. Use superseding corrections rather than rewriting the record, except for explicitly required privacy redaction.
...
At handoff, compare each row with actual artifacts and the current task record. Use `forge-recall` for any verified, authorized task-specific history. Missing transcript access does not justify scanning unrelated tasks.

- Follow evidence pointers and confirm the stated result.
- Correct invented, inaccurate or superseded claims with a new row.
- Add material missing decisions, pivots and blockers without padding the log.
- Keep secrets and private data out of the log; redact them when required.
